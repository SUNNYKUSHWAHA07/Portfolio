...
import {vertexShader , fragmentShader} from './projectShader.js';
...
const createTexttureAtlas = (textures, isText = false) => {
...
const updateMousePosition = (event) => {
    ...
    mousePostion.x = event.clientX - rect.left;
    mousePostion.y = event.clientY - rect.top;
...
const handelMove = (currentX, currentY) => {
...
const onPointerMove = (e) => handelMove(e.clientX, e.clientY);
const onTouchMove = (e) => {
    e.preventDefault();
    handelMove(
        e.touches[0].clientX,
...
const setupEventListeners = () => {
    ...
    renderer.domElement.addEventListener("mouseleave", ()=>{
      mousePostion.x = mousePostion.y = -1;
...
const init = async () => {
    ...
    const imageAtlas = createTexttureAtlas(imageTextures, false);
    const textAtlas =  createTexttureAtlas(textTextures, true);
